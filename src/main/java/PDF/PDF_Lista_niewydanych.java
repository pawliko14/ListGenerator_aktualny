...
public class PDF_Lista_niewydanych {
	...
	public static void create(){
		System.out.println("Start taktowa");
		...
		try
		{
			...
			String name = "";
			name = "Lista niewydanych artykulow.pdf";
			...
			String sql = "Select nrMaszyny, opis, klient, dataprodukcji, dataKoniecMontazu, komentarz from calendar where Zakonczone = 0 and nrMaszyny not like '6/%' order by dataKoniecMontazu";
			...
			while(ProjectSchedule.next()){
				...
				if(dodatkowy){
					sql0 += " or (PROJECTNUMMER like '"+innyNumer+"%' and BOSTDEH <> 0) ";
				}
				...
				String sql01 = "SELECT "+Columns+" from storenotesdetail " +
						"where (PROJECTNUMMER LIKE '"+calyNumer+"%' and BOSTDEH <> 0) ";
				if(dodatkowy){
					sql01+= " or (PROJECTNUMMER like '"+innyNumer+"%' and BOSTDEH <> 0) ";
				}
					...
					if(RowNumber == 1){
						while(!Lista500.isEmpty()){
							...
							ResultSet rs2 = st2.executeQuery("SELECT "+Columns+" from storenotesdetail " +
									"where projectnummer = '500/"+art.substring(4, art.length())+"' and BOSTDEH <> 0 " +
							...
							while(rs2.next()){
								...
								Statement znajdz500 = connection.createStatement();
								ResultSet Piecset = znajdz500.executeQuery("SELECT MatSource, `ConsumerOrder` FROM partsoverview WHERE `OrderNo`= '"+ProjectGroup+"/"+ProjectNumber+"' AND `ItemNo` = '"+rs2.getString("ARTIKELCODE")+"'");
								
								if(Piecset.next()){
									if(Piecset.getString("MatSource").startsWith("500/")){
										Piecsetka=Piecset.getString("MatSource");
										subproject = Piecset.getString(2);
									}
								}
		...
		System.out.println("Koniec taktowa");
	}
